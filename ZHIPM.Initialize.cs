...
namespace ZHIPlayerManager
{
	public partial class ZHIPM : TerrariaPlugin
	{
		...
		/// <summary>
		/// 排行榜REST推送，类型有online、coin、fish、boss、kill、rarenpc、point、death、clumsy
		/// </summary>
		/// 
		private object RankData(RestRequestArgs args)
		{
			string type = args.Parameters["type"];
			int count = 100;
			if (type == "online")//在线时长排行
			{
				...
				List<OnlineRank> rankdata = new List<OnlineRank>();
				if (count > list.Count)
				{
					count = list.Count;
				}
				...
				{
					rankdata.Add(new OnlineRank { Name= list[i].Name ,time= list[i].time});
				}
			...
			else if (type == "coin")//钱币排行
			{
				...
				for (int i = 0; i < count; i++)
				{
					coin.Add(new CoinRank { Name = list[i].Name, coin = getPlayerCoin(list[i].Name) });
			...
			else if (type == "fish")//钓鱼排行
			{
				List<UserAccount> list = new List<UserAccount>();
				List<timesRank> fish= new List<timesRank>();
				...
				for (int i = 0; i < count; i++)
				{
					fish.Add(new timesRank { Name = list[i].Name, times = TShock.CharacterDB.GetPlayerData(new TSPlayer(-1), list[i].ID).questsCompleted });
				}
				return new RestObject()
				{
					{
			...
			{			
				...
				List<timesRank> rankdata = new List<timesRank>();
				if (count > list.Count)
				{
					count = list.Count;
				}
				for (int i = 0; i < count; i++)
				{
					rankdata.Add(new timesRank { Name = list[i].Name, times = getKillNumFromDictionary(list[i].killBossID) });
				}
				return new RestObject()
				{
					{
			...
			else if (type == "kill")//杀死生物排行
			{
				...
				for (int i = 0; i < count; i++)
				{
					rankdata.Add(new timesRank { Name = list[i].Name, times = list[i].killNPCnum });
				}
				return new RestObject()
				{
					{
			...
			else if (type == "rarenpc")//杀死罕见生物排行
			{
				...
				for (int i = 0; i < count; i++)
				{
					rankdata.Add(new timesRank { Name = list[i].Name, times = getKillNumFromDictionary(list[i].killRareNPCID) });
				}
				return new RestObject()
				{
					{
			...
			else if (type == "point")//点数排行
			{
				...
				List<PointsRank> rankdata = new List<PointsRank>();
				if (count > list.Count)
				{
					count = list.Count;
				}
				for (int i = 0; i < count; i++)
				{
					rankdata.Add(new PointsRank { Name = list[i].Name, points = list[i].point });
				}
				return new RestObject()
				{
					{
			...
			else if (type == "death")//死亡次数排行
			{
				...
				for (int i = 0; i < count; i++)
				{
					rankdata.Add(new timesRank { Name = list[i].Name, times = list[i].deathCount });
				}
				return new RestObject()
				{
					{
			...
			else if (type == "clumsy")//菜鸡程度
			{
				...
				List<NoobRank> rankdata = new List<NoobRank>();
				if (count > list.Count)
				{
					count = list.Count;
				}
				for (int i = 0; i < count; i++)
				{
					rankdata.Add(new NoobRank { Name = list[i].Name, point = list[i].deathCount * 1000.0 / list[i].time });
				}
				return new RestObject()
				{
					{
